...
from src.train_module.training import weights_init
...
def main(finetune: bool, feature_model: str, relation_model: str, learning_rate: int,
    ...
    feature_encoder, feature_encoder_optim, feature_encoder_scheduler, relation_network, relation_network_optim, relation_network_scheduler = init_encoder_and_network_for_training(
        feature_model, finetune, gpu, learning_rate, load_imagenet, nbr_episode, relation_model, use_gpu, class_num)
    ...
    for episode in range(start_episode, nbr_episode):
        ...
        samples, sample_labels, batches, batch_labels, chosen_classes = get_training_batch(class_num,
                                                                                           ...
                                                                                           batch_num_per_class,
                                                                                           train_data_path)
        ft_list, relation_pairs = get_relation_pairs_and_encoded_features_multiclass(batch_num_per_class, batches, class_num,
...
def get_relation_pairs_and_encoded_features_multiclass(batch_num_per_class, batches, class_num, feature_encoder, gpu,
    ...
    sample_features = sample_features.view(class_num, sample_num_per_class, 512, 7, 7)  # (N,K,512,7,7)
    sample_features = torch.sum(sample_features, 1).squeeze(1)  # (N,512,7,7)
    if use_gpu:
    ...
    # calculate relations
    sample_features_ext = sample_features.unsqueeze(0).repeat(batch_num_per_class * class_num, 1, 1, 1, 1) # (K,N,512,7,7)
...
def init_encoder_and_network_for_training(feature_model, finetune, gpu, learning_rate, load_imagenet, nbr_episode,
                                          relation_model, use_gpu, class_num):
    feature_encoder = CNNEncoder(pretrained=load_imagenet, class_num=class_num)
